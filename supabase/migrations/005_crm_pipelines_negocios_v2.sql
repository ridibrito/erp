-- =====================================================
-- NEXUS ERP - CRM: PIPELINES E NEGÓCIOS (V2)
-- Migração: 005_crm_pipelines_negocios_v2.sql
-- Data: 2025-09-30
-- =====================================================
...
USING (
    org_id::text = (SELECT org_id::text FROM public.profiles WHERE id::text = auth.uid())
);
...
USING (
    pipeline_id IN (
        ...
        WHERE org_id::text = (SELECT org_id::text FROM public.profiles WHERE id::text = auth.uid())
    )
);
