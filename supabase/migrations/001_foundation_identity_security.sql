...
-- Migração: 001_foundation_identity_security.sql
-- Data: 2024-01-XX
-- =====================================================
...
CREATE INDEX idx_tenant_domains_org_id ON tenant_domains(org_id);
CREATE INDEX idx_tenant_domains_status ON tenant_domains(status);

-- Índices para tenant_settings
CREATE INDEX idx_tenant_settings_org_id ON tenant_settings(org_id);
...
CREATE INDEX idx_user_permissions_user_org ON user_permissions(user_id, org_id);
...
CREATE INDEX idx_data_consents_user_org ON data_consents(user_id, org_id);
CREATE INDEX idx_data_consents_purpose ON data_consents(purpose);

-- Índices para data_subject_requests
CREATE INDEX idx_data_subject_requests_user_org ON data_subject_requests(user_id, org_id);
...
CREATE INDEX idx_security_logs_org_id ON security_logs(org_id);
...
CREATE INDEX idx_oauth_connections_user_org ON oauth_connections(user_id, org_id);
...
COMMENT ON TABLE tenant_domains IS 'Domínios customizados por organização para multi-tenancy';
COMMENT ON TABLE tenant_settings IS 'Configurações específicas por tenant';
COMMENT ON TABLE permissions IS 'Permissões granulares do sistema';
COMMENT ON TABLE roles IS 'Papéis de usuário por organização';
COMMENT ON TABLE role_permissions IS 'Relacionamento entre papéis e permissões';
COMMENT ON TABLE user_permissions IS 'Permissões especiais por usuário';
COMMENT ON TABLE user_sessions IS 'Sessões ativas de usuário';
COMMENT ON TABLE two_factor_auth IS 'Configuração de autenticação de dois fatores';
COMMENT ON TABLE trusted_devices IS 'Dispositivos confiáveis para 2FA';
COMMENT ON TABLE data_consents IS 'Consentimentos LGPD por usuário';
COMMENT ON TABLE data_subject_requests IS 'Solicitações do titular dos dados (LGPD)';
COMMENT ON TABLE data_processing_logs IS 'Log de processamento de dados pessoais';
COMMENT ON TABLE audit_logs IS 'Log de auditoria de todas as ações';
COMMENT ON TABLE security_logs IS 'Log de eventos de segurança';
COMMENT ON TABLE api_keys IS 'Chaves de API para integrações';
COMMENT ON TABLE oauth_connections IS 'Conexões OAuth com provedores externos';
